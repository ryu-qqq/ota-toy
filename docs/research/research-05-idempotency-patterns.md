...
### 2.2 해외 OTA(B사) Connectivity API

해외 OTA(B사)은 Pull 방식의 예약 연동을 사용한다. 일반적인 REST API 예약 생성과는 다른 패턴이다.

**예약 프로세스 (OTA 연동):**

```
1. 공급자가 GET OTA_HotelResNotif 폴링 (20초 간격 권장)
2. 해외 OTA(B사)이 신규 예약 데이터 반환
...
> **출처:** [해외 OTA(B사) - Understanding the OTA reservations process](https://developers.booking.com/connectivity/docs/reservations-api/reservations-process-ota)
...
| **해외 OTA(B사)** | Pull 폴링 + ACK | response_token + ACK | 해당 없음 (B2B) | 폴링 기반 구조적 멱등 |
...
- [해외 OTA(B사) - OTA Reservations Process](https://developers.booking.com/connectivity/docs/reservations-api/reservations-process-ota)
