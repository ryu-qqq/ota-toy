...
> 도메인 코드와 정합성 동기화 완료. 전 테이블에 audit/soft-delete 컬럼 포함.
...
erDiagram
    ...
    SupplierTask {
        long id PK
        long supplier_id FK
        long supplier_api_config_id FK
        string task_type
        string status
        text payload
        int retry_count
    ...
    ReservationSession ||--o| Reservation : confirms
    Reservation ||--o{ ReservationLine : contains
    ReservationLine ||--o{ ReservationItem : contains
    ReservationLine }o--|| RatePlan : references
    ...
    Supplier ||--o{ SupplierRawData : fetches
    Supplier ||--o{ SupplierTask : schedules
