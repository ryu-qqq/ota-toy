...
#### dependency-guardian (인프라 엔지니어)

**왜 필요한가:** 멀티모듈 Gradle 프로젝트에서 각 builder 에이전트가 제각각 build.gradle.kts에 의존성을 추가하면 **버전 충돌, scope 오류, Hexagonal 의존 방향 위반**이 발생한다. convention-guardian이 ArchUnit의 유일 수정자인 것처럼, dependency-guardian은 **빌드 파일의 유일 수정자**다.

**하는 일:**
- `gradle/libs.versions.toml` 버전 카탈로그 중앙 관리
- 모든 `build.gradle.kts`, `settings.gradle.kts`, `gradle.properties` 소유/수정
- 다른 에이전트의 DEP-REQUEST 검토 (필요성, 모듈 적합성, scope, 버전 충돌, 라이선스)
- 정적 분석 플러그인(Spotless, Checkstyle, PMD, SpotBugs) 설정/관리
- 새 모듈 추가 시 build.gradle.kts 생성

**도구 권한:** Read, Write, Glob, Grep, Bash (빌드 검증용)

**피드백 루프 — DEP-REQUEST/DEP-RESPONSE:**
- 구현팀이 의존성 필요 시 DEP-REQUEST 발행 → dependency-guardian이 검토 → APPROVED/REJECTED
- domain 모듈에 Spring/JPA 추가 시도 → 즉시 거절 (Hexagonal 위반)
...
| **빌드 파일 수정** | dependency-guardian만 | build.gradle.kts, libs.versions.toml 등 빌드 설정은 전체 모듈에 영향. 여러 에이전트가 수정하면 버전 충돌/의존 방향 위반이 발생한다 |
