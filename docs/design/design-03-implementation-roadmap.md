...
> 초안 작성일: 2026-04-02 | 최종 갱신: 2026-04-06
...
### 구현 완료
- **Domain**: 14개 BC — property, roomtype, brand, propertytype, roomattribute, pricing, inventory, reservation, partner, member, supplier, location, accommodation(공유 타입), common(공유 VO)
- **Application**: UseCase, Service, Manager, Validator, Factory, Assembler, Facade, Port 전체 구현 (137개 파일)
- **Adapter-In**: Extranet 9개 + Customer 5개 API 엔드포인트, GlobalExceptionHandler, ErrorMapper
- **Adapter-Out**: JPA Entity 28개, Flyway 마이그레이션 16개, Redis Lua 스크립트 3개
- **Infra**: Docker Compose (MySQL + Redis), SpringDoc/Swagger, 부트스트랩 3개 (Extranet:8080, Customer:8081, Admin:8082)
- **테스트**: Domain 46개, Application 12개, Persistence 10개, REST API 7개, ArchUnit 14개+

### Phase 1~8 전체 완료
- 아래 Phase별 상세 설명은 **초기 계획 기록**으로, 실제 구현은 모두 완료되었습니다.
...
1. **ERD에 Outbox 테이블 추가** — 완료 (ReservationOutbox, SupplierOutbox, SupplierTask)
2. **Money VO 방향 결정** — BigDecimal 기반으로 결정 (Money(BigDecimal) 채택)
3. **domain-convention 패키지 구조 갱신** — 14개 BC로 세분화 완료
